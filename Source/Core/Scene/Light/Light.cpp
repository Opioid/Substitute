...
float Light::falloff_epsilon() const
{
...
float3 Light::directional_energy() const
{
	return lumen_ * color_;
}

float4 Light::point_energy_and_range() const
{
	return energy_and_range(4.f * math::pi);
}

float4 Light::spot_energy_and_range() const
{
	return energy_and_range(2.f * math::pi * (1.f - cos(fov_ * 0.5f)));
}
...
{
	float4 energy_and_range = spot_energy_and_range();
...
const float4x4& Light::texture_transform()
{
...
float4 Light::energy_and_range(float solid_angle) const
{
