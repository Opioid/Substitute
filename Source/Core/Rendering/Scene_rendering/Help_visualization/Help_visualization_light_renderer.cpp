...
bool Help_visualization_light_renderer::init(Resource_manager& resource_manager, Constant_buffer_cache& /*constant_buffer_cache*/)
{
	...
	light_probe_technique_  = effect_->get_technique("Light_probe");

	input_layout_ = rendering_tool_.get_vertex_layout_cache().get_input_layout(*Vertex_position3x32_tex_coord2x32_normal1010102::vertex_layout_description(), light_probe_technique_->get_program()->get_signature());
	if (!input_layout_)
	{
		return false;
	}
...
void Help_visualization_light_renderer::render(const scene::Scene& scene, const Rendering_context& context)
{
	...
	device.set_vertex_buffers(sphere_model_->get_vertex_layout_description()->get_num_streams(),
							  sphere_model_->get_vertex_buffers(),
							  sphere_model_->get_vertex_layout_description()->get_strides());

	device.set_index_buffer(sphere_model_->get_index_buffer());
...
void Help_visualization_light_renderer::render(const scene::Irradiance_volume& volume, const scene::Camera& camera)
{
	...
	change_per_light_.get_data().light_data = invert(camera.get_view()) * invert(world);
	change_per_light_.update(device);

	device.set_shader_resources(scene::Irradiance_volume::get_num_textures(), volume.get_textures());

	uint32_t num_probes = volume.get_num_probes();
	for (uint32_t i = 0; i < num_probes; ++i)
	{
		render_diffuse_probe(volume.get_probe_position(i), camera.get_view());
	}
}
...
void Help_visualization_light_renderer::render_diffuse_probe(const float3& position, const float4x4& view)
{
	...
	device.draw_indexed(sphere_model_->get_num_indices());
}
