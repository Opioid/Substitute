...
Control_mappings& Controls::get_mappings()
{
	return mappings_;
}
...
void Controls::update(float speed)
{
	while (signal_queue_.is_signal_left())
	{
        ...
        {
			mouse_.set_axis(id - platform::motion::Mouse_x, signal.get_data());
        }
        else if (id < 256)
        {
            if (signal.is_pressed())
...
const Input_state& Controls::get_mouse_state() const
{
	return mouse_;
}

const Input_state& Controls::get_keyboard_state() const
{
	return keyboard_;
}


const Controls::Signal_stream& Controls::get_signal_stream() const
{
