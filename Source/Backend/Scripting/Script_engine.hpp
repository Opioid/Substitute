...
namespace scripting
{
	...
	public:
		...
		template<typename T>
		void set_message_callback(void (T::*mp)(const std::string&, Message::Value), T* callee)
		{
		...
		bool garbage_collect(int flags = Garbage_collection::Full_cycle) const;
		...
		bool register_function(const std::string& name, asSFuncPtr const& function, Calling_convention::Value calling_convention = Calling_convention::C_decl) const;
