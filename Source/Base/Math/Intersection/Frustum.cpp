...
Intersection_type::Value Frustum::intersect(const OBB& obb) const
{
	for (size_t i = 0; i < 6; ++i)
	{
		...
		if (m < 0.f)
		{
			float n = std::abs(dot(planes_[i].get_normal(), obb.m_halfsize_x)) + std::abs(dot(planes_[i].get_normal(), obb.m_halfsize_y)) + std::abs(dot(planes_[i].get_normal(), obb.m_halfsize_z));
